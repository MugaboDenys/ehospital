...
const Dashboard = () => {
  ...
  const meds = user.pharmacistMeds;
  console.log(meds, "++++++++++++++++++++++++++++");
  console.log(user, "<<<<<<<<<<<<<<<<<<<<<<<<<<<");
  useEffect(() => {
  ...
  const handleDownload = async () => {
    try {
      const body = {
        medName: meds[0].medName,
        medPrice: meds[0].medPrice,
        expirationDate:meds[0].expirationDate,
      };
    ...
    <div className="relative bg-gradient-to-b from-teal-100/40 to-teal-800/40 min-h-screen overflow-hidden">
      ...
      >
        ...
        <DialogBody divider>
          ...
          <h2 className="font-bold pt-5">Suggested Medecines :</h2>
          {meds?.map((item, index) => (
            <div key={index} className="flex gap-5 justify-center">
              <div>
                <h2 className="font-bold">Medecine Name</h2>
                <p>{item.medName}</p>
              </div>
              <div>
                <h2 className="font-bold">Medecine Price</h2>
                <p>{item.medPrice}</p>
              </div>
              <div>
                <h2 className="font-bold">Medecine Expiration Date</h2>
                <p>{item.expirationDate.replace(/ AM$/, "")}</p>
              </div>
            </div>
          ))}
        </DialogBody>
      ...
      {role !== "patient" && (
        <h2 className="title uppercase pt-5 text-teal-500 text-center font-extrabold text-3xl">
          {` ${role}s`}
        </h2>
      )}
